...
datos_c <- datos_c %>% 
  mutate(Apellidos=ifelse(datos$Apellidos =="DE LA",word(IS,1,3),datos_c$Apellidos),
         Apellidos=ifelse(datos$Apellidos == "LIZARAN MACEDA",paste(word(IS,-2,-1),word(IS,1)),datos_c$Apellidos))
...
# Menú Más ----------------------------------------------------------------
                   navbarMenu("Más",
                              tabPanel("Usuario",
                                       ...
                                       fluidRow(#align = "center",
                                                      ...
                                                      column(12,actionButton("ask","Cambiar Contraseña",icon = icon("edit"), style = "color: #fff; background-color: #F89438; padding:20px; font-size:100%"),align = "center"),
  ...
  datos_filtrados1 <- reactive({
    if (user()$user == "admin"){
      d <- datos
    }else {#if (user()$user == "xerox"){
      d <- datos %>% 
        filter(
          str_detect(Cliente, users %>% filter(user == user()$user) %>% pull(cliente))
        )
      # filter(
      #     Cliente %like% as.character(user_info$user)
      # )
    }
    return(d)
  })
